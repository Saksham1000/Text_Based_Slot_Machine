...
def get_slot_machine_spin(rows, cols, symbols):
    all_symbols = []
    for symbol, symbol_countcount in symbols.items():
        for _ in range(symbol_countcount):
...
def deposit():
    while True:
        amount = input("What would you like to deposite ? Rs")
        if amount.isdigit():
            amount = int(amount)
            if amount > 0:
                break
            else:
                print("Amount must be graater than 0.")
        else:
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(f"You do not have enough balance to place this bet. Your current balance is Rs{balance}.")
        else:
             break
...
def main():
    ...
    print(f"You left with Rs{balance}. Thanks for playing!")
